...
class DB_Actions extends DB_Connect
{
    ...
    public function save_entry()
    {
        ...
        try
        {
            ...
            FB::log($stmt->errorInfo(), "PDOStatement::errorInfo()");
            $stmt->closeCursor();
    ...
    protected function get_entry_by_id($id)
    {
        ...
        $cache = Utilities::check_cache($sql.$id);
        ...
        try
        {
            ...
            return $this->_load_entry_array($stmt);
        }
        catch ( Exception $e )
        {
            $this->_log_exception($e);
    ...
    protected function get_entry_by_url( $url=NULL )
    {
        ...
        // Check for a cached file
        $cache = Utilities::check_cache($sql.$url);
        ...
        // Cache the data
        $file = Utilities::save_cache($sql.$url, $this->entries);
    }
    ...
    protected function get_entries_by_tag($category, $offset=0)
    {
        ...
        try
        {
            ...
            $data = $this->_load_entry_array($stmt);
        }
        catch ( Exception $e )
        {
            $this->_log_exception($e);
        ...
        // Cache the data
        $file = Utilities::save_cache($cache_id, $data);
    }
    ...
    protected function get_entries_by_search( $search, $offset=0 )
    {
		...
		try
        {
            ...
            // Breaks the query into search keys
            $keys = explode(' ', $query);
            $key_search = NULL;
            foreach( $keys as $key )
            {
            ...
            $this->_load_entry_array($stmt, TRUE);
        }
        catch ( Exception $e )
        {
    ...
    protected function get_all_entries( $offset=0, $ord="`created` DESC" )
    {
        ...
        // Check for a cached file
        $cache = Utilities::check_cache($sql.$this->url0.$ord.$offset.$this->entry_limit);
        if( $cache!==FALSE && strlen($cache)>0 )
        {
            $this->entries = $cache;
            return;
        ...
        // Cache the data
        $file = Utilities::save_cache($sql.$this->url0, $this->entries);
    }
    ...
    protected function get_entry_count_by_search($search)
    {
        ...
        $sql = "SELECT
                    COUNT(`title`) AS `the_count`
                FROM `".DB_NAME."`.`".DB_PREFIX."entries`
                WHERE MATCH (`title`,`entry`,`excerpt`,`tags`)
