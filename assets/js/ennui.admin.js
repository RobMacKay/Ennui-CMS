...
$(".ecms-edit").live("click", function(e){
        ...
        showedit(page, 'entry-edit', id);
    });

// Handler to auto-generate slugs
$("input#title").live("keydown", function(){
        $("input#slug").val(make_url($(this).val()));
    })
    .live("blur", function(){
        $("input#slug").val(make_url($(this).val()));
    });
...
{
    var token = $("meta[name=ecms:token]").attr("content"),
        params = "page=" + page + "&action=" + option + "&entry_id=" + id
            + "&token=" + token;
...
function textEdit()
{
    $('textarea#entry')
        .tinymce({
                ...
                theme : "advanced",
                plugins : "safari,iespell,inlinepopups,spellchecker,paste,"
                        + "advimage,media",
