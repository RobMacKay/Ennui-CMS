...
//                alert("Something unexpected is in here! ");
        ...
        function displayThumbbox(el, src, imgCap)
        {
            var main = $('<div>')
                ...
                thumbs = el.parent('ul').find('a').clone(),
                ul = $('<ul>')
                    .css({
                            "left":"0px",
                            "width":el.parent('ul').children('li').length*(parseInt(o.previewWidth)
                                    +parseInt(o.border)/2)+parseInt(o.border)/2+"px"
                        })
                    .html(thumbs),
                max_images = Math.min(
                        el.parent('ul').children('li').length,
    ...
    $.fn.thumbBox.buildModal = function(data, options)
    {
        var o = $.extend($.fn.thumbBox.defaults, options),
            close = $("<a>")
                .addClass("thumbbox-close-btn")
		...
		$('<div>')
			...
			.css({
                ...
                "width":parseInt(o.fullWidth)+"px",
                "height":"auto",
				"margin-left":parseInt(o.fullWidth)/-2+"px"
			})
			.html('<div class="thumbbox-main-modal">'+data+'</div>')
            .css({"opacity":"0"})
            .append(close)
			.appendTo("html");
    ...
    $.fn.thumbBox.setColors = function(options)
    {
        ...
        $(".thumbbox-main,.thumbbox-main-img-caption")
            .css({
                    "background" : dark,
                    "background" : "rgba("+darkRGB[0]+","+darkRGB[1]+","
                            +darkRGB[2]+",.8)"
                });
